...
def deptwise_data(dept_name):
    ...
    try:
        ...
        # print("rep_select_Query- ", rep_users)
        ...
        # print("busy_users- ", busy_users)
        busy_users = [list(tp) for tp in busy_users]
        # print("busy_log_record- ", busy_users)
        ...
        # print("new_log_record- ", new_users)
...
def dept_register(request):
    ...
    # user_ = DepartmentAdminUser.objects.get(user=reg_user)
    # depart_name = user_.department
    # admin_type = user_.usertype
    # print("admin_type", admin_type)
    # print("depart_name", depart_name)
...
def get_dept_data(request, department_name):
    ...
    if department_name not in updated_list:
        ...
        total_users_cnt = Log.objects.all().values('user_email').filter(intent=department_name).distinct().count()
        eng_users_cnt = Log.objects.all().values('user_email').distinct().count()
        repeated_users_list = Log.objects.all().values('user_email', 'user_datetime', 'intent')

        # print("counts_data=> ", wr_answer_cnt, rt_answer_cnt, no_answer_cnt, live_cnt, reset_cnt, total_users_cnt,
        #       eng_users_cnt)
        ## bar and line chart
        bar_data, line_data, rep_data, busy_data, new_user_data = dept_chart(department_name)
